...
void read_link(string type, struct pid_info info){
	...
	// any file was accesible -> type = "unknown"
		...
		// from special link
			...
			// special deleted! -> change name, type
			if(strstr(info.name.c_str(), "deleted") != NULL){
				info.name = info.name.substr(0, info.name.length()-10);
				info.type = "unknown";
			}
		}
	}
...
void parse_map(struct pid_info info){
    ...
    while (getline(mapfile, str))
    {
		...
		// map deleted! -> change fd, name, type
        if(strstr(info.name.c_str(), "deleted") != NULL) 
        {
            info.fd = "DEL";
			info.name = info.name.substr(0, info.name.length()-10);
...
void open_dirp(char* pid){
	...
	if (fd < 0){ // any error > skip this process
		return;
	}
	char buf[1024];
	int n = read(fd, buf, sizeof(buf));
	close(fd);
	if(n < 0){ // any error > skip this process
		return;
	}
										// buf[n-1] = '\0';
	info.cmd = buf;
    ...
    if(!stat(info.path.c_str(), &pid_stat)){
		...
		else{ // any error > skip this process
			return;
		}
    }
